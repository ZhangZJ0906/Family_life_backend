...
public interface SubscriptionDao extends JpaRepository<Subscription, Integer> {
	...
	// 查詢自己
	@Query(value = "SELECT * FROM subscriptions WHERE group_id = 0 and user_id = :userId ORDER BY next_billing_date ASC", nativeQuery = true)
	List<Subscription> findBySelfId(@Param("userId") Long userId);

	// 找訂閱名字
	@Query(value = "SELECT name FROM subscriptions WHERE id = :Id", nativeQuery = true)
	String getOldNameById(@Param("Id") Integer Id);
	...
	@Query(value = "INSERT INTO subscriptions "
			...
			+ "VALUES "
			+ "(:groupId, :userId, :name, :price, :billingCycle, :nextBillingDate, :purchaseDate, :trialEndDate, :notify, :note, :status, :remindMessage, :createdAt)", nativeQuery = true)
