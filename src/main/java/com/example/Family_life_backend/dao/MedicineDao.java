...
public interface MedicineDao extends JpaRepository<Medicine, Integer> {
	...
	@Query(value = "SELECT * FROM medicines WHERE group_id = :groupId ORDER BY expire_date ASC", nativeQuery = true)
	List<Medicine> findByGroupId(@Param("groupId") Integer groupId);

	// 查詢所屬群組
	@Query(value = "SELECT group_id FROM medicines WHERE id = :Id", nativeQuery = true)
	Long getThisGroup(@Param("Id") Integer Id);

	// 查詢自己
	@Query(value = "SELECT * FROM medicines WHERE group_id = 0 and user_id = :userId ORDER BY expire_date ASC", nativeQuery = true)
	List<Medicine> findBySelfId(@Param("userId") Long userId);

	// 找藥名
	@Query(value = "SELECT name FROM medicines WHERE id = :Id", nativeQuery = true)
	...
	@Query(value = "INSERT INTO medicines " + "(group_id, user_id, name, medicine_type, quantity, unit, safe_quantity, "
			...
			+ "location, source, notify, note, unit_price, price, status, remind_message,avatar) " + "VALUES "
			+ "(:groupId, :userId, :name, :medicineType, :quantity, :unit, :safeQuantity, "
			+ ":purchaseDate, :expireDate, :dosage, :usageMethod, "
			+ ":location, :source, :notify, :note, :unitPrice, :price, :status, :remindMessage , :avatar )", nativeQuery = true)
	int addMedicine(@Param("groupId") Integer groupId, @Param("userId") Integer userId, @Param("name") String name,
			...
			@Param("expireDate") LocalDate expireDate, @Param("dosage") String dosage,
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE medicines SET " + "group_id = :groupId, " + "user_id = :userId, " + "name = :name, "
			+ "medicine_type = :medicineType, " + "quantity = :quantity, " + "unit = :unit, "
			...
			+ "status = :status, " + "remind_message = :remindMessage , avatar = :avatar "
	...
	int updateMedicine(@Param("id") Integer id, @Param("groupId") Integer groupId, @Param("userId") Integer userId,
			...
			@Param("unit") String unit, @Param("safeQuantity") Integer safeQuantity,
